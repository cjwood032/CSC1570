...
def main():
    ...
    try:
        ...
        contents = num_file.readline() # read the first line's contents
        while contents!='': #set a loop to see if the contents are emptly
            amount = float(contents) #convert a string to a float
            total += amount #get the total
            lines += 1 #increase the counter
            contents=num_file.readline() #read the next line's contents
